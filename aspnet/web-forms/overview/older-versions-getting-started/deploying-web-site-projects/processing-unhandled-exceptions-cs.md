...
[Affichez ou téléchargez l’exemple de code](https://github.com/dotnet/AspNetDocs/tree/master/aspnet/web-forms/overview/older-versions-getting-started/deploying-web-site-projects/processing-unhandled-exceptions-cs/samples) ([procédure de téléchargement](/aspnet/core/tutorials/index#how-to-download-a-sample))
...
> [!NOTE]
> Lors du déploiement de l’application ASP.NET, vous devez copier le fichier `Global.asax` dans l’environnement de production. Le fichier `Global.asax.cs`, qui est créé dans le WAP, n’a pas besoin d’être copié en production, car ce code est compilé dans l’assembly du projet.
