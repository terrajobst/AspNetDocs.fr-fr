...
title: Mise à jour parC#lots () | Microsoft Docs
...
ms.locfileid: "74583387"
---
# <a name="batch-updating-c"></a>Mise à jour par lots (C#)

par [Scott Mitchell](https://twitter.com/ScottOnWriting)

[Télécharger le code](https://download.microsoft.com/download/3/9/f/39f92b37-e92e-4ab3-909e-b4ef23d01aa3/ASPNET_Data_Tutorial_64_CS.zip) ou [Télécharger le PDF](batch-updating-cs/_static/datatutorial64cs1.pdf)
...
[![chaque ligne dans le GridView est modifiable](batch-updating-cs/_static/image1.gif)](batch-updating-cs/_static/image1.png)

**Figure 1**: chaque ligne du contrôle GridView est modifiable ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image2.png))

Commençons !

> [!NOTE]
> Dans le didacticiel sur l' [exécution des mises à jour par lot](../editing-and-deleting-data-through-the-datalist/performing-batch-updates-cs.md) , nous avons créé une interface de modification par lot à l’aide du contrôle DataList. Ce didacticiel diffère de l’ancien dans qui utilise un GridView et la mise à jour par lot est effectuée dans l’étendue d’une transaction. À la fin de ce didacticiel, je vous encourage à revenir au didacticiel précédent et à le mettre à jour pour utiliser les fonctionnalités liées aux transactions de base de données ajoutées dans le didacticiel précédent.
...
[![configurer ObjectDataSource pour utiliser la classe ProductsBLL](batch-updating-cs/_static/image2.gif)](batch-updating-cs/_static/image3.png)

**Figure 2**: configurer ObjectDataSource pour utiliser la classe `ProductsBLL` ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image4.png))

[![récupérer les données du produit à l’aide de la méthode GetProducts](batch-updating-cs/_static/image3.gif)](batch-updating-cs/_static/image5.png)

**Figure 3**: récupérer les données du produit à l’aide de la méthode `GetProducts` ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image6.png))
...
[![définir les listes déroulantes dans les onglets mettre à jour, insérer et supprimer sur (aucun)](batch-updating-cs/_static/image4.gif)](batch-updating-cs/_static/image7.png)

**Figure 4**: définir les listes déroulantes des onglets mettre à jour, insérer et supprimer sur (aucun) ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image8.png))
...
![Convertir le BoundFields et le CheckBoxField existants en TemplateField](batch-updating-cs/_static/image5.gif)

**Figure 5**: convertir le BoundFields et le CheckBoxField existants en TemplateField
...
[![le TemplateField ProductName comprend désormais une zone de texte et un contrôle RequiredFieldValidator](batch-updating-cs/_static/image6.gif)](batch-updating-cs/_static/image9.png)

**Figure 6**: le `ProductName` TemplateField comprend maintenant une zone de texte et un contrôle RequiredFieldValidator ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image10.png))
...
[![ajouter un CompareValidator pour vous assurer que le prix entré est une valeur monétaire non négative](batch-updating-cs/_static/image7.gif)](batch-updating-cs/_static/image11.png)

**Figure 7**: ajouter un CompareValidator pour vous assurer que le prix entré est une valeur monétaire non négative ([cliquez pour afficher l’image en plein écran](batch-updating-cs/_static/image12.png))
...
[![configurer ObjectDataSource pour utiliser la classe CategoriesBLL](batch-updating-cs/_static/image8.gif)](batch-updating-cs/_static/image13.png)

**Figure 8**: configurer ObjectDataSource pour utiliser la classe `CategoriesBLL` ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image14.png))
...
**Figure 9**: récupérer les données de catégorie à l’aide de la méthode `GetCategories` ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image16.png))
...
[![définir les listes déroulantes dans les onglets mettre à jour et supprimer sur (aucun)](batch-updating-cs/_static/image10.gif)](batch-updating-cs/_static/image17.png)

**Figure 10**: définir les listes déroulantes des onglets mettre à jour et supprimer sur (aucun) ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image18.png))
...
[![lier le DropDownList à CategoriesDataSource](batch-updating-cs/_static/image11.gif)](batch-updating-cs/_static/image19.png)

**Figure 11**: lier le DropDownList à l' `CategoriesDataSource` ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image20.png))
...
![Lier la valeur RéfCatégorie du produit à la propriété SelectedValue s de DropDownList](batch-updating-cs/_static/image12.gif)
...
![Mettre en forme la propriété Text en tant que nombre](batch-updating-cs/_static/image14.gif)
...
![Centrer la case à cocher abandonnée](batch-updating-cs/_static/image15.gif)
...
[![la page comprend désormais deux boutons mettre à jour les produits](batch-updating-cs/_static/image16.gif)](batch-updating-cs/_static/image23.png)

**Figure 16**: la page comprend désormais deux boutons mettre à jour les produits ([cliquez pour afficher l’image en taille réelle](batch-updating-cs/_static/image24.png))

## <a name="step-5-updating-the-products"></a>Étape 5 : mise à jour des produits

Lorsqu’un utilisateur visite cette page, il effectue ses modifications, puis clique sur l’un des deux boutons mettre à jour les produits. À ce stade, nous devons enregistrer les valeurs entrées par l’utilisateur pour chaque ligne dans une instance de `ProductsDataTable`, puis les transmettre à une méthode BLL qui transmet ensuite cette `ProductsDataTable` instance à la méthode de `UpdateWithTransaction` DAL. La méthode `UpdateWithTransaction`, que nous avons créée dans le [didacticiel précédent](wrapping-database-modifications-within-a-transaction-cs.md), garantit que le lot de modifications sera mis à jour en tant qu’opération atomique.
...
Tout d’abord, un appel est effectué pour `BatchUpdate`. Ensuite, le `ClientScript property` est utilisé pour injecter du code JavaScript qui affichera un MessageBox qui lit les produits qui ont été mis à jour.
