...
[![attacher fichier NORTHWND. Base de données MDF à partir du dossier App_Data](using-sql-cache-dependencies-vb/_static/image2.gif)](using-sql-cache-dependencies-vb/_static/image1.png)

**Figure 2**: attacher la base de données `NORTHWND.MDF` à partir du dossier `App_Data` ([cliquez pour afficher l’image en taille réelle](using-sql-cache-dependencies-vb/_static/image2.png))
...
[![définir la liste déroulante sur (aucun) pour les onglets insérer et supprimer](using-sql-cache-dependencies-vb/_static/image7.gif)](using-sql-cache-dependencies-vb/_static/image7.png)
...
[![l’événement ObjectDataSource s Selection Trigger chaque fois que le contrôle GridView est paginé, modifié ou trié](using-sql-cache-dependencies-vb/_static/image8.gif)](using-sql-cache-dependencies-vb/_static/image9.png)

**Figure 8**: l’événement ObjectDataSource s `Selecting` se déclenche chaque fois que le contrôle GridView est paginé, modifié ou trié ([cliquez pour afficher l’image en taille réelle](using-sql-cache-dependencies-vb/_static/image10.png))

Comme nous l’avons vu dans la [mise en cache des données avec le didacticiel ObjectDataSource](caching-data-with-the-objectdatasource-vb.md) , la définition de la propriété `EnableCaching` sur `True` amène ObjectDataSource à mettre en cache ses données pour la durée spécifiée par sa propriété `CacheDuration`. ObjectDataSource possède également une [propriété`SqlCacheDependency`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource.sqlcachedependency.aspx), qui ajoute une ou plusieurs dépendances de cache SQL aux données mises en cache à l’aide du modèle :
...
**Figure 11**: configurer ObjectDataSource pour utiliser la classe `ProductsCL` ([cliquez pour afficher l’image en taille réelle](using-sql-cache-dependencies-vb/_static/image16.png))
...
**Figure 12**: sélectionner la méthode `GetProducts` dans la liste déroulante Sélectionner les onglets ([cliquez pour afficher l’image en taille réelle](using-sql-cache-dependencies-vb/_static/image18.png))

[![choisir la méthode UpdateProduct dans la liste déroulante des onglets mettre à jour](using-sql-cache-dependencies-vb/_static/image13.gif)](using-sql-cache-dependencies-vb/_static/image19.png)
