...
Pour utiliser la fonctionnalité de démarrage automatique, un administrateur IIS définit un pool d’applications dans IIS 7,5 pour qu’il soit démarré automatiquement à l’aide de `applicationHost.config` la configuration suivante dans le fichier:

[!code-xml[Main](overview/samples/sample5.xml)]

Étant donné qu’un seul pool d’applications peut contenir plusieurs applications, vous spécifiez des applications individuelles qui doivent être démarrées automatiquement `applicationHost.config` à l’aide de la configuration suivante dans le fichier:

[!code-xml[Main](overview/samples/sample6.xml)]

Lorsqu’un serveur IIS 7,5 est démarré à froid ou lorsqu’un pool d’applications est recyclé, IIS 7,5 utilise les informations `applicationHost.config` du fichier pour déterminer les applications Web qui doivent être démarrées automatiquement. Pour chaque application marquée pour un démarrage automatique, IIS 7.5 envoie une requête à ASP.NET 4 pour démarrer l’application dans un État pendant lequel l’application n’accepte pas temporairement les demandes HTTP. Lorsqu’il est dans cet État, ASP.NET instancie le type défini par l’attribut *serviceAutoStartProvider* (comme indiqué dans l’exemple précédent) et appelle dans son point d’entrée public.
...
Avec l’ajout simple du nouvel attribut au `Web.config` fichier, les applications avec des cycles d’UC de rechange sur les serveurs Web peuvent réaliser des réductions substantielles de la taille des données d’état de session sérialisées.
...
Par défaut, l’attribut *requestPathInvalidCharacters* définit huit caractères comme étant non valides. (Dans la chaîne qui est assignée à *requestPathInvalidCharacters* par défaut, les caractères&lt;inférieur à (),&gt;supérieur à () et&amp;perluète () sont encodés, car `Web.config` le fichier est un fichier XML.) Vous pouvez personnaliser le jeu de caractères non valides en fonction des besoins.

> [!NOTE]
> Remarque ASP.NET 4 rejette toujours les chemins d’accès d’URL qui contiennent des caractères de la plage ASCII 0x00 à 0x1F, car il s’agit de caractères d’URL non valides[http://www.ietf.org/rfc/rfc2396.txt](http://www.ietf.org/rfc/rfc2396.txt), tels que définis dans le document RFC 2396 de l’IETF (). Sur les versions de Windows Server qui exécutent IIS 6 ou version ultérieure, le pilote de périphérique de protocole http. sys rejette automatiquement les URL avec ces caractères.
...
Dans ASP.NET 4, la fonctionnalité de validation de demande a été rendue extensible afin que vous puissiez utiliser la logique de validation de requête personnalisée. Pour étendre la validation de la demande, vous créez une classe qui dérive du nouveau type *System. Web. util. RequestValidator* et vous configurez l’application (dans la `Web.config` section httpRuntime du fichier) pour utiliser le type personnalisé. L’exemple suivant montre comment configurer une classe de validation de demande personnalisée:
...
Vous pouvez créer un encodeur personnalisé en dérivant du nouveau type *System. Web. util. HttpEncoder* , puis en configurant ASP.net pour utiliser le type personnalisé dans la `Web.config` section httpRuntime du fichier, comme illustré dans l’exemple suivant:
...
ASP.NET 4 tire parti de la nouvelle fonctionnalité d’analyse des ressources introduite par le CLR. Pour activer cette fonctionnalité, vous pouvez ajouter l’extrait de code de configuration XML `aspnet.config` suivant au fichier de configuration.

[!code-xml[Main](overview/samples/sample16.xml)]

> [!NOTE]
> Notez que `aspnet.config` le fichier se trouve dans le répertoire d’installation de l' .NET Framework. Ce n’est pas `Web.config` le fichier.
...
Le réseau de distribution de contenu (CDN) Microsoft Ajax vous permet d’ajouter facilement des scripts ASP.NET AJAX et jQuery à vos applications Web. Par exemple, vous pouvez commencer à utiliser la bibliothèque jQuery simplement en ajoutant `<script>` une balise à votre page qui pointe vers Ajax.Microsoft.com comme suit:

[!code-html[Main](overview/samples/sample19.html)]

En tirant parti du CDN Microsoft Ajax, vous pouvez améliorer considérablement les performances de vos applications Ajax. Le contenu du CDN Microsoft AJAX est mis en cache sur les serveurs situés dans le monde entier. En outre, le CDN Microsoft Ajax permet aux navigateurs de réutiliser les fichiers JavaScript mis en cache pour les sites Web qui se trouvent dans des domaines différents.
...
1. S’il n’y a pas de balises *méta* dans l’élément *Head* qui correspondent aux noms de propriétés (autrement dit, Name = "keywords" pour *page.* MetaTags et Name = "Description" pour *page. meta description*, ce qui signifie que ces propriétés n’ont pas été définies) ), les balises *meta* seront ajoutées à la page lors de son rendu.
...
Le contenu de la balise *meta* Description est utilisé pour améliorer les aperçus de la liste de recherche dans Google. (Pour plus d’informations, consultez [améliorer les extraits de code avec une méta-Description permodernisation](http://googlewebmastercentral.blogspot.com/2007/09/improve-snippets-with-meta-description.html) sur le blog Google Webmaster central.) Google et Windows Live Search n’utilisent pas le contenu des mots clés pour tout, mais d’autres moteurs de recherche peuvent l’être. Pour plus d’informations, consultez les conseils sur les [Mots clés META](http://www.searchengineguide.com/richard-ball/meta-keywords-a.php) sur le site Web du Guide du moteur de recherche.
...
Activée`: [DynamicValue]`

Désactivé`[DynamicValue]`

Toutefois, après une publication (postback), la sortie suivante s’affiche:

Activée`: [DeclaredValue]`

Désactivé`[DynamicValue]`

Le contrôle Label1 (dont la valeur *ViewStateMode* est définiesur Disabled) n’a pas conservé la valeur à laquelle il a été défini dans le code. Toutefois, le contrôle Label2 (dont la valeur *ViewStateMode* est définie sur *activé*) a conservé son état.
...
- Au niveau de l’ordinateur, vous créez ou mettez `.browser` à jour un fichier XML dans le dossier suivant:
...
- Pour une application individuelle, vous créez un `.browser` fichier dans le dossier de `App_Browsers` l’application.

Ces approches nécessitent la modification des fichiers XML et, pour les modifications au niveau de l’ordinateur, vous devez redémarrer l’application après avoir\_exécuté le processus ASPNET RegBrowsers. exe.
...
En pratique, les développeurs ne définissent pas souvent des fonctionnalités de navigateur personnalisées. Les fichiers browser sont difficiles à mettre à jour, le processus de mise à jour est assez complexe et la syntaxe `.browser` XML pour les fichiers peut être complexe à utiliser et à définir. Ce processus serait beaucoup plus simple, s’il existait une syntaxe de définition de navigateur commune, ou une base de données qui contenait des définitions de navigateur à jour, ou même un service Web pour une telle base de données. La nouvelle fonctionnalité des fournisseurs de fonctionnalités de navigateur rend ces scénarios possibles et pratiques pour les développeurs tiers.
...
2. Inscrivez le fournisseur auprès de l’application. 
    ...
    Ce code doit s’exécuter dans l’événement de démarrage de `Global.asax` l' *application\_* du fichier. Toute modification apportée à la classe *BrowserCapabilitiesProvider* doit se produire avant l’exécution du code de l’application, afin de s’assurer que le cache reste dans un état valide pour l’objet *HttpCapabilitiesBase* résolu.
...
La méthode décrite dans la section précédente montre comment obtenir des données de routage dans le code d’une page de Web Forms. Vous pouvez également utiliser des expressions dans le balisage qui vous permettent d’accéder aux mêmes informations. Les générateurs d’expressions sont un moyen puissant et élégant d’utiliser du code déclaratif. (Pour plus d’informations, consultez l’entrée [Express avec](http://haacked.com/archive/2006/11/29/Express_Yourself_With_Custom_Expression_Builders.aspx) les générateurs d’expressions personnalisées sur le blog de Phil Haack.)
...
L’option Range est similaire à l’option de recherche, mais elle spécifie une paire de valeurs pour définir la plage. Dans l’exemple suivant, le contrôle *QueryExtender* recherche la `UnitPrice` colonne dans les données retournées à partir du contrôle *LinqDataSource* . La plage est lue à partir des contrôles TextBoxFrom et TextBoxTo sur la page.
...
Certains sites ASP.net (surtout avec ASP.NET MVC) s’appuient fortement `<%` sur l’utilisation =  `expression %>` de la syntaxe (souvent appelée «code pépites») pour écrire du texte dans la réponse. Lorsque vous utilisez des expressions de code, il est facile d’oublier le codage HTML du texte, si le texte provient d’une entrée d’utilisateur, il peut laisser des pages ouvertes à une attaque XSS (cross site scripting).
...
Par exemple, &lt;%: La demande ["UserInput"]&gt; % effectue un encodage HTML sur la valeur de la *demande ["UserInput"]* .
...
Dans les versions antérieures de ASP.net, lorsque vous utilisez Visual Studio pour créer un projet de site Web ou un projet d’application Web, les projets résultants contiennent uniquement une page default. `Web.config` aspx, un fichier `App_Data` par défaut et le dossier, comme indiqué dans l’exemple suivant: suivante
...
Le projet comprend un certain nombre de fichiers qui n’ont pas été créés dans des versions antérieures. En outre, le nouveau projet d’application Web est configuré avec la fonctionnalité d’appartenance de base, qui vous permet de commencer rapidement à sécuriser l’accès à la nouvelle application. En raison de cette inclusion, `Web.config` le fichier du nouveau projet comprend des entrées utilisées pour configurer l’appartenance, les rôles et les profils. L’exemple suivant montre le `Web.config` fichier d’un nouveau projet d’application Web. (Dans ce cas, *roleManager* est désactivé.)

[![](overview/_static/image13.png)](overview/_static/image12.png)

([Cliquez pour afficher l’image en taille réelle](overview/_static/image14.png))

Le projet contient également un deuxième `Web.config` fichier dans le `Account` répertoire. Le deuxième fichier de configuration fournit un moyen de sécuriser l’accès à la page ChangePassword. aspx pour les utilisateurs non connectés. L’exemple suivant montre le contenu du deuxième `Web.config` fichier.
...
Par exemple, imaginez que pour une nouvelle application Web, vous souhaitez modifier certaines des couleurs et insérer le logo de votre société à la place du logo de l’application My ASP.NET. Pour ce faire, vous devez créer un répertoire sous `Content` pour stocker l’image de votre logo:

<a id="0.2_graphic23"></a>![](overview/_static/image19.png)

Pour ajouter l’image à la page, ouvrez le `Site.Master` fichier, recherchez l’emplacement où est défini le texte de l’application My ASP.net, puis remplacez-le par un élément *image* dont l’attribut *src* a pour valeur la nouvelle image de logo, comme dans l’exemple suivant:
...
En plus des améliorations du rendu, la navigation au clavier du menu a été améliorée grâce à la gestion du focus. Quand le contrôle de *menu* reçoit le focus, vous pouvez utiliser les touches de direction pour naviguer parmi les éléments. Le contrôle de *menu* attache désormais également des rôles et des attributs Aria (Rich Internet applications) accessibles Vale[aile du](http://www.w3.org/TR/wai-aria-practices/#menu "menu instructions Aria")pour une meilleure accessibilité.
...
Le `EntityTemplate` répertoire contient des modèles permettant d’afficher des objets de modèle de données. Par défaut, les objets sont rendus à `Default.ascx` l’aide du modèle, qui fournit le balisage qui ressemble exactement au balisage créé par le contrôle *DetailsView* utilisé par Dynamic Data dans ASP.net 3,5 SP1. L’exemple suivant illustre le balisage pour `Default.ascx` le contrôle:
...
Les nouveaux modèles d’entité s’affichent sur une page à l’aide du nouveau contrôle *DynamicEntity* . Au moment de l’exécution, ce contrôle est remplacé par le contenu du modèle d’entité. Le balisage suivant montre le contrôle *FormView* dans `Detail.aspx` le modèle de page qui utilise le modèle d’entité. Notez l’élément *DynamicEntity* dans le balisage.
...
ASP.net 4 introduit deux nouveaux modèles de champ intégrés, `EmailAddress.ascx` et. `Url.ascx` Ces modèles sont utilisés pour les champs marqués comme *EmailAddress* ou *URL* avec l’attribut *DataType* . Pour les objets *EmailAddress* , le champ est affiché sous la forme d’un lien hypertexte créé à l’aide du protocole *mailto:* . Lorsque les utilisateurs cliquent sur le lien, il ouvre le client de messagerie de l’utilisateur et crée un message squelette. Les objets de type *URL* sont affichés sous forme de liens hypertexte ordinaires.
...
Ce balisage crée un lien qui pointe vers la page de liste `Products` pour la table en fonction des itinéraires définis dans `Global.asax` le fichier. Le contrôle utilise automatiquement le nom de la table par défaut sur lequel est basée la page Dynamic Data.

<a id="0.2__Toc224729047"></a><a id="0.2__Toc253429284"></a><a id="0.2__Toc243304655"></a>

### <a name="support-for-inheritance-in-the-data-model"></a>Prise en charge de l’héritage dans le modèle de données

Le Entity Framework et le LINQ to SQL prennent en charge l’héritage dans leurs modèles de données. Il peut s’agir, par exemple, d’une base `InsurancePolicy` de données qui contient une table. Il peut également contenir `CarPolicy` des `HousePolicy` tables et qui ont les mêmes champs `InsurancePolicy` que, puis ajouter d’autres champs. Dynamic Data a été modifiée pour comprendre les objets hérités dans le modèle de données et pour prendre en charge la génération de modèles automatique pour les tables héritées.
...
La Entity Framework offre une prise en charge complète des relations plusieurs-à-plusieurs entre les tables, qui est implémentée en exposant la relation en tant que collection sur un objet d' *entité* . De `ManyToMany.ascx` nouveaux `ManyToMany_Edit.ascx` modèles de champs et ont été ajoutés pour prendre en charge l’affichage et la modification des données impliquées dans des relations plusieurs-à-plusieurs.
...
Visual Studio 2010 inclut plus de 200 extraits de code qui vous aident à compléter automatiquement les balises ASP.NET et HTML courantes, y compris les attributs requis (tels que runat = "Server") et les attributs courants spécifiques d’une balise (par exemple, *ID*, *DataSourceID*,  *ControlToValidate*et *Text*).
...
- [https://www.asp.net/dynamicdata/](https://msdn.microsoft.com/library/cc488545.aspx)et [ASP.NET Dynamic Data plan du contenu](https://msdn.microsoft.com/library/cc488545%28VS.100%29.aspx) — ressources en ligne sur le site de l’équipe ASP.net et dans la documentation officielle de ASP.NET Dynamic Data.
