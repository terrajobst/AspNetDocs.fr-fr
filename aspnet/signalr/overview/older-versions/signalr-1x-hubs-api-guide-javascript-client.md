...
---
# <a name="signalr-1x-hubs-api-guide---javascript-client"></a>Guide de l’API SignalR 1.x Hubs - Client JavaScript

de [Patrick Fletcher](https://github.com/pfletcher), [Tom Dykstra](https://github.com/tdykstra)

[!INCLUDE [Consider ASP.NET Core SignalR](~/includes/signalr/signalr-version-disambiguation.md)]

> Ce document fournit une introduction à l’utilisation de l’API hubs pour Signalr version 1,1 dans les clients JavaScript, tels que les navigateurs et les applications du Windows Store (WinJS).
> 
...
Dans l’exemple précédent, la référence au proxy généré par Signalr consiste à générer dynamiquement du code JavaScript, et non un fichier physique. Signalr crée le code JavaScript pour le proxy à la volée et le transmet au client en réponse à l’URL « /signalr/hubs ». Si vous avez spécifié une autre URL de base pour les connexions Signalr sur le serveur dans votre méthode `MapHubs`, l’URL du fichier proxy généré dynamiquement est votre URL personnalisée avec « /hubs » ajouté.
...
Dans une vue Razor ASP.NET MVC 4, utilisez le tilde pour faire référence à la racine de l’application dans votre référence de fichier proxy :

[!code-html[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample5.html)]

Pour plus d’informations sur l’utilisation de Signalr dans MVC 4, consultez [prise en main avec signalr et MVC 4](tutorial-getting-started-with-signalr-and-mvc-4.md).
...
Quand vous exécutez un projet Web dans Visual Studio 2012 en mode débogage et que vous utilisez Internet Explorer comme navigateur, vous pouvez voir le fichier proxy dans **Explorateur de solutions** sous **documents de script**, comme indiqué dans l’illustration suivante.
...
En général, si le navigateur charge une page à partir de `http://contoso.com`, la connexion Signalr se trouve dans le même domaine, à `http://contoso.com/signalr`. Si la page de `http://contoso.com` établit une connexion à `http://fabrikam.com/signalr`, il s’agit d’une connexion inter-domaines. Pour des raisons de sécurité, les connexions inter-domaines sont désactivées par défaut. Pour établir une connexion inter-domaines, assurez-vous que les connexions inter-domaines sont activées sur le serveur et spécifiez l’URL de connexion lorsque vous créez l’objet de connexion. Signalr utilise la technologie appropriée pour les connexions inter-domaines, telles que [JSONP](http://en.wikipedia.org/wiki/JSONP) ou [cors](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing).

Sur le serveur, activez les connexions inter-domaines en sélectionnant cette option lorsque vous appelez la méthode `MapHubs`.
...
**Code client qui spécifie une connexion inter-domaines (avec le proxy généré)**

[!code-javascript[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample11.js?highlight=1)]

**Code client qui spécifie une connexion inter-domaines (sans le proxy généré)**

[!code-javascript[Main](signalr-1x-hubs-api-guide-javascript-client/samples/sample12.js?highlight=1)]

Lorsque vous utilisez le constructeur `$.hubConnection`, vous n’avez pas besoin d’inclure `signalr` dans l’URL, car il est ajouté automatiquement (sauf si vous spécifiez `useDefaultUrl` comme `false`).
...
> 
>     Signalr gère l’utilisation de JSONP ou CORS. L’affectation de la valeur true à `jQuery.support.cors` désactive JSONP car Signalr peut supposer que le navigateur prend en charge CORS.
